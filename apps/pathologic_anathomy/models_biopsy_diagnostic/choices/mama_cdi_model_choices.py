...
class TipoHistologicoChoices(models.IntegerChoices):
    ...
    CARCINOMA_DUCTAL_INFILTRANTE_SOE = (
        1,
    ...
    CARCINOMA_INFILTRANTE_CON_CAMBIOS_LOBULARES = (
        4,
        "Carcinoma infiltrante con cambios lobulares",
    )
    CARCINOMA_INFILTRANTE_CON_CAMBIOS_DUCTALES_Y_LOBULARES = (
        5,
    ...
    CARCINOMA_INVASIVO_VARIANTE_TUBULO_LOBULAR = (
        8,
        "Carcinoma invasivo, variante tubulo-lobular",
    )
    ...
    CARCINOMA_INVASIVO_CON_CAMBIOS_MEDULARES = (
        13,
        "Carcinoma invasivo con cambios medulares",
    )
    ...
    CARCINOMA_FIBROMATOSIS_LIKE_METAPLASTIC = (
        16,
        "Carcinoma fibromatosis-like metaplastic",
    )
    CARCINOMA_METAPLASICO_SPINDLE_CELL_TYPE = (
        17,
        "Carcinoma metaplàsico, spindle cell type",
    )
    CARCINOMA_METAPLASICO_TIPO_MIXTO_EPITHELIAL_Y_MESENQUIMATOSO = (
        18,
        "Carcinoma metaplàsico typo mixto epithelial and mesenquimatoso",
    )
    CARCINOMA_INVASIVE_CON_CAMBIOS_METAPLASICO = (
        19,
        "Carcinoma Invasive con cambios metaplàsico",
    )
    ...
    CARCINOMA_INVASIVO_CON_CAMBIOS_APOCRINOS = (
        22,
        "Carcinoma invasivo con cambios apocrinos",
    )
    CARCINOMA_INVASIVO_CON_CAMBIOS_CLEAR_CELL = (
        23,
        "Carcinoma invasivo concambios clear cell (glycogen rich)",
    )
    CARCINOMA_INVASIVO_WITH_NEUROENDOCRINE_FEATURES = (
        24,
        "Carcinoma invasivo with neuroendocrine features",
    )
    CARCINOMA_INVASIVE_CON_CELULAS_EN_ANILLO_DE_SELLO_FEATURES = (
        25,
        "Carcinoma invasive con células en anillo de sello features",
    )
    CARCINOMA_SECRETOR = 26, "Carcinoma secretor"
    CARCINOMA_INVASIVE_TYPE_CANNOT_BE_DETERMINED = (
        27,
        "Carcinoma Invasive, type cannot be determined",
    )
...
class ExtensionTumoralPielChoices(models.IntegerChoices):
    # Define las opciones del modelo usando el formato (valor, etiqueta)
    INVADE_DIRECTAMENTE_SIN_ULCERACION = (
        1,
        "Invade directamente la dermis o epidermis sin ulceración",
    )
    INVADE_DIRECTAMENTE_CON_ULCERACION = (
        2,
...
class EfectosTratamientoChoices(models.IntegerChoices):
    ...
    NO_SE_DEFINE_RESPUESTA_A_TRATAMIENTO_PRE_QUIRURGICO = (
        2,
        "No se define respuesta a tratamiento pre quirúrgico",
    )
    RESPUESTA_PROBABLE_O_DEFINIDA_A_TRATAMIENTO_PRE_QUIRURGICO = (
        3,
        "Respuesta probable o definida a tratamiento pre quirúrgico",
    )
    NO_CARCINOMA_RESIDUAL_INVASOR_LUEGO_DE_TRATAMIENTO_PRE_QQCO = (
        4,
...
class EfectosTratamientoGLChoices(models.IntegerChoices):
    ...
    NO_SE_DEFINE_RESPUESTA_A_TTO_PRE_QUIRURGICO = (
        2,
        "No se define respuesta a tto pre quirúrgico",
    )
    RESPUESTA_PROBABLE_O_DEFINIDA_RESPONSE_A_TTO_PRE_QUIRURGICO = (
        3,
        "Respuesta probable o definida response a tto pre quirúrgico",
    )
    NO_GL_METASTASICOS_CICATRIZ_FIBROSA_POSIBLEMENTE_EN_RELACION_CON_METASTASIS_CON_RESPUESTA_PATOLOGICA_COMPLETA = (
        4,
        "No GL metastásicos. Cicatriz fibrosa posiblemente en relación con metástasis con respuesta patológica completa.",
    )
    NO_GL_METASTASICOS_Y_NO_CICATRIZ_FIBROSA = (
        5,
        "No GL metastásicos y no cicatriz fibrosa",
    )
